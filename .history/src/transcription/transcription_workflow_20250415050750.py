...
def transcribe_audio(audio_path: str) -> str:
    ...
    Uses direct HTTP POST with requests, not the OpenAI Python client.
    """
    import wave
    import math
    import tempfile
    ...
    OPENROUTER_WHISPER_ENDPOINT = "https://openrouter.ai/api/v1/audio/transcriptions"  # Confirm this endpoint in OpenRouter docs
    WHISPER_MODEL = (
        "openai/whisper-large-v3"  # Confirm this model name in OpenRouter docs
    )

    if not OPENROUTER_API_KEY:
        raise ConnectionError("OPENROUTER_API_KEY not set in environment.")
    ...
    def transcribe_chunk(chunk_path):
        with open(chunk_path, "rb") as audio_file:
            ...
            if response.status_code != 200:
                logging.error(
                    f"OpenRouter Whisper API error for {chunk_path}: {response.status_code} - {response.text}"
                )
                raise RuntimeError(
    ...
    if duration <= max_chunk_seconds:
        ...
        logging.info(
            f"Starting transcription for {audio_path} via OpenRouter (requests)..."
        )
        try:
