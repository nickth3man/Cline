...
def transcribe_audio(audio_path: str) -> str:
    ...
    if duration <= max_chunk_seconds:
        ...
        logging.info(
            f"Starting transcription for {audio_path} via OpenAI Whisper API..."
        )
        try:
            raw_transcript = transcribe_chunk(audio_path)
        except Exception as e:
            logging.error(
                f"OpenAI Whisper API error for {audio_path}: {e}", exc_info=True
            )
    ...
    try:
        ...
        chat_completion = client.with_options(timeout=300.0).chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a helpful assistant that formats transcripts with speaker labels.",
                },
