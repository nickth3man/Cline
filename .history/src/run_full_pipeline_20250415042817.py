        ...
        for model_id in correction_models:
            try:
                corrected = correct_transcript(
                    raw_transcript, diarization_result, correction_model=model_id
                )
                corrected_transcripts[model_id] = corrected
                corrected_path = os.path.join(
                    video_folder,
                    f"{sanitized_title}_corrected_{model_id.replace('/', '_')}.md",
                )
                with open(corrected_path, "w", encoding="utf-8") as f:
                    f.write(corrected)
                logging.info(
                    f"Corrected transcript saved for model {model_id}: {corrected_path}"
                )
            except Exception as e:
                logging.error(
                    f"Correction failed for model {model_id} on video {video_title}: {e}"
                )

        # Summarize transcript using summarization model
        # Use the first corrected transcript if available, else raw transcript
        transcript_to_summarize = next(
            iter(corrected_transcripts.values()), raw_transcript
        )
        try:
            summary = workflow_logic.summarize_transcript(
                transcript_to_summarize, summarization_model
            )
