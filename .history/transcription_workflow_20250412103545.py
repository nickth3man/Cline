...
def main() -> int:
    ...
    manage_group.add_argument(
        "--list-errors", action="store_true", help="List videos with an error status"
    )
    manage_group.add_argument(
        "--view",
        type=str,
    ...
    is_manage_run = any(
        [args.status, args.list_errors, args.view, args.retry, args.delete]
    )
    is_export_run = bool(args.export)

    if is_pipeline_run and (is_manage_run or is_export_run):
        parser.error(
            "Cannot combine pipeline execution arguments (--playlist_url) with database management or export arguments."
        )
    if is_manage_run and is_export_run:
        parser.error(
            "Cannot combine database management arguments with export arguments."
        )
    if not is_pipeline_run and not is_manage_run and not is_export_run:
        parser.error(
            "No action specified. Provide --playlist_url, a management command (e.g., --status), or an export command (--export)."
        )
    if args.retry and args.retry.lower() != "all-errors" and not args.retry:
        parser.error(
            "--retry requires a specific VIDEO_ID or the keyword 'all-errors'."
        )
    ...
    final_transcription_model = args.transcription_model or config.get(
        "defaults", {}
    ).get(
        "transcription_model",
        model_manager.DEFAULT_CONFIG["defaults"]["transcription_model"],
    )
    final_correction_model = args.correction_model or config.get("defaults", {}).get(
        "correction_model", model_manager.DEFAULT_CONFIG["defaults"]["correction_model"]
    )
    ...
    try:
        ...
        logging.critical(
            f"Failed to initialize Database Manager for path {db_path}: {e}",
            exc_info=True,
        )
        return 1
    ...
    if args.status:
        ...
        try:
            ...
            logging.error(
                f"Failed to retrieve details for video {video_id_to_view}: {e}",
                exc_info=True,
            )
        ...
        if target.lower() == "all-errors":
            ...
            try:
                ...
                logging.error(
                    f"Failed to retrieve error videos for retry: {e}", exc_info=True
                )
        else:
            ...
            try:
                ...
                logging.error(
                    f"Failed to retrieve video {target} for retry: {e}", exc_info=True
                )
        ...
        if videos_to_retry:
            ...
            for video in videos_to_retry:
                ...
                retry_status = (
                    "downloaded"  # Default: assume audio exists, retry processing
                )
                paths = manager.get_video_paths(video.video_id)
                if (
                    not paths
                    or not paths.audio_wav_path
                    or not os.path.exists(
                        os.path.join(args.work_dir, paths.audio_wav_path)
                    )
                ):
                    retry_status = (
                        "pending_download"  # Audio missing, needs full redownload
                    )
                ...
                if manager.update_video_status(video.video_id, retry_status):
                    ...
                    title_display = (
                        (video.title[:30] + "...")
                        if video.title and len(video.title) > 30
                        else (video.title or "N/A")
                    )
                    print(
                        f" - Marked {video.video_id} ({title_display}) with status '{retry_status}'"
                    )
                    count += 1
                else:
        ...
        print(
            f"Attempting to delete database record for video ID: {video_id_to_delete}"
        )
        ...
        try:
            ...
            if not paths:
                ...
                confirm_files = input(
                    f"Database record found for '{paths.video_id}'. Also delete associated files in folder '{paths.folder_path}'? (y/N): "
                )
                deleted_paths = manager.delete_video(video_id_to_delete)

                if deleted_paths:
                    print(
                        f"Successfully deleted database record for {video_id_to_delete}."
                    )
                    if confirm_files.lower() == "y":
                        print("Attempting to delete associated files...")
                        folder_to_delete = (
                            os.path.join(args.work_dir, deleted_paths.folder_path)
                            if deleted_paths.folder_path
                            else None
                        )
                        ...
                        if folder_to_delete and os.path.isdir(folder_to_delete):
                            try:
                                ...
                                print(
                                    f"Successfully deleted folder: {folder_to_delete}"
                                )
                            except OSError as e:
                                print(f"Error deleting folder {folder_to_delete}: {e}")
                                logging.error(
                                    f"Error deleting folder {folder_to_delete}: {e}",
                                    exc_info=True,
                                )
                        else:
                            print(
                                f"Folder path '{folder_to_delete}' not found or invalid, cannot delete files."
                            )
                else:
                    print(f"Failed to delete database record for {video_id_to_delete}.")
        except Exception as e:
            logging.error(
                f"Failed to delete video {video_id_to_delete}: {e}", exc_info=True
            )
    ...
    if args.export:
        ...
        try:
            rows_exported = 0
            if file_ext == ".csv":
                rows_exported = manager.export_to_csv(
                    filepath, video_ids=video_ids, status=status_filter
                )
                print(
                    f"Successfully exported {rows_exported} records to CSV: {filepath}"
                )
            elif file_ext == ".jsonl" or file_ext == ".json":
                if file_ext == ".json":
                    logging.warning(
                        "Exporting as JSON Lines (.jsonl) format to the specified .json file for streaming efficiency."
                    )
                rows_exported = manager.export_to_json(
                    filepath, video_ids=video_ids, status=status_filter
                )
                print(
                    f"Successfully exported {rows_exported} records to JSON Lines: {filepath}"
                )
            else:
                print(
                    f"Error: Unsupported export file format '{file_ext}'. Please use '.csv' or '.jsonl'."
                )
    ...
    if is_pipeline_run and not management_action_taken and not export_action_taken:
        ...
        try:
            ...
            logging.info(
                "Pipeline processing steps complete. Check the output directory and database for results."
            )
        except Exception as e:
        ...
        not is_pipeline_run and not management_action_taken and not export_action_taken
    ):
        # This case should be caught by argument validation, but as a fallback:
        logging.warning(
            "No pipeline URL provided and no management or export action specified."
        )
        parser.print_help()
